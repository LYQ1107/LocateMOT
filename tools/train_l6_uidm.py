...
class UIDMRollout:
    ...
    def __init__(self, model, clips, device, teacher=True,
        ...
        self.loss_acc = {}
        self.n_frames = 0
        self.seeded = any(c.get("seeded", False) for c in clips)
        if self.seeded:
            self._initialize_seeded_states(clips)

    def _initialize_seeded_states(self, clips):
        """Initialize seeded identities with the same birth token as eval.

        Prompt-seeded clips enter with an existing identity state rather
        than learning a NEW transition at frame zero.  The construction is
        deliberately identical to ``tools/eval_l12_davis.py::seed_h``:
        PBD encoder + zero candidate evidence + unified observation adapter
        followed by the UIDM memory initializer.  Rows are assembled with
        ``stack`` instead of in-place writes so adapter/core gradients can
        flow through the seed state during joint fine-tuning.
        """
        if not self.unified or not hasattr(self.raw, "adapter"):
            raise ValueError("seeded clips require the unified L8/L9 model")
        from locatemot.models.l1d_association import CAND_FEATURES

        d = self.raw.d_model
        app_dim = getattr(self.raw, "app_dim", 2048)
        clip_dim = self.raw.adapter.clip_proj.mlp[0].in_features
        spec_dim = self.raw.adapter.spec_proj.mlp[0].in_features
        h_rows, anchor_rows = [], []
        ref_rows, anchor_pbd_rows = [], []
        alive_rows, active_rows = [], []
        last_rows, prev_rows = [], []
        seen_rows, age_rows, hit_rows, slot_rows = [], [], [], []

        for b, clip in enumerate(clips):
            h_row = [torch.zeros(d, device=self.device) for _ in range(self.S)]
            anchor_row = [torch.zeros(d, device=self.device)
                          for _ in range(self.S)]
            ref_row = [torch.zeros(app_dim, device=self.device)
                       for _ in range(self.S)]
            anchor_pbd_row = [torch.zeros(app_dim, device=self.device)
                              for _ in range(self.S)]
            alive = torch.full((self.S,), -5.0, device=self.device)
            active = torch.zeros(self.S, dtype=torch.bool, device=self.device)
            last = torch.zeros(self.S, 4, device=self.device)
            prev = torch.zeros_like(last)
            seen = torch.zeros(self.S, dtype=torch.long, device=self.device)
            age = torch.zeros(self.S, dtype=torch.long, device=self.device)
            hits = torch.zeros(self.S, dtype=torch.long, device=self.device)
            slots = torch.full((self.S,), -1, dtype=torch.long,
                               device=self.device)
            spec = torch.as_tensor(
                clip["spec"], dtype=torch.float32, device=self.device)
            for seed in clip.get("seeds", []):
                gid = seed["gid"]
                si = self.gid_maps[b].get(gid)
                if si is None or si >= self.S:
                    continue
                sp = torch.as_tensor(seed["pbd"], dtype=torch.float32,
                                     device=self.device)
                sc = torch.as_tensor(seed.get("clip", np.zeros(clip_dim)),
                                     dtype=torch.float32,
                                     device=self.device)
                pbd_tok = self.raw.uidm.pbd_encoder(sp.unsqueeze(0))
                zero_feat = torch.zeros(
                    1, len(CAND_FEATURES), device=self.device)
                sem, _ = self.raw.adapter(
                    sp.unsqueeze(0), sc.unsqueeze(0), spec.unsqueeze(0))
                tok = pbd_tok + self.raw.uidm.cand_mlp(zero_feat) + sem
                hs = self.raw.uidm.memory.init(tok).squeeze(0)
                h_row[si] = hs
                # Anchors are evidence snapshots, not optimization paths.
                anchor_row[si] = hs.detach()
                ref_row[si] = sp
                anchor_pbd_row[si] = sp
                active[si] = True
                # eval_l12_davis injects the seed through OnlineTracker's
                # forced-birth path, whose initial UIDM alive state is 1.0.
                # Keep the training seeded state on that same lifecycle
                # scale; the first learned association happens on frame 1.
                alive[si] = 1.0
                box = torch.as_tensor(seed["box"], dtype=torch.float32,
                                     device=self.device)
                last[si] = box
                prev[si] = box
                age[si] = 1
                hits[si] = 1
                slots[si] = int(si)
            h_rows.append(torch.stack(h_row))
            anchor_rows.append(torch.stack(anchor_row))
            ref_rows.append(torch.stack(ref_row))
            anchor_pbd_rows.append(torch.stack(anchor_pbd_row))
        ...
        self.h = torch.stack(h_rows)
        self.anchor = torch.stack(anchor_rows)
        self.ref_pbd = torch.stack(ref_rows)
        self.anchor_pbd = torch.stack(anchor_pbd_rows)
        self.alive_logit = torch.stack(alive_rows)
        self.active = torch.stack(active_rows)
        self.last_box = torch.stack(last_rows)
        self.prev_box = torch.stack(prev_rows)
        self.last_seen = torch.stack(seen_rows)
        self.age = torch.stack(age_rows)
        self.hits = torch.stack(hit_rows)
        # gid_maps already assign contiguous ids; slot_gt is the target gid.
        for b, clip in enumerate(clips):
            for seed in clip.get("seeds", []):
                si = self.gid_maps[b].get(seed["gid"])
                if si is not None and si < self.S:
                    self.slot_gt[b, si] = int(si)
    ...
    def _step(self, clips, f):
        ...
        if self.unified:
            ...
            spec_valid = torch.zeros(B, dtype=torch.float32, device=dev)
            for b, c in enumerate(clips):
                if c.get("spec") is not None:
                    spec[b] = torch.as_tensor(c["spec"], dtype=torch.float32,
                                              device=dev)
                    spec_valid[b] = float(c.get("spec_valid", 1.0))
        ...
        if not self.teacher:
            # AMP may leave these inference-only decode tensors in BF16/FP16;
            # SciPy/Hungarian expects a NumPy-supported floating type.
            pair_all = pred["pair_logits"].detach().float().cpu().numpy()
            nm_all = pred["no_match"].detach().float().cpu().numpy()
            nw_all = pred["new"].detach().float().cpu().numpy()
        for b, c in enumerate(clips):
            fr = c["frames"][f]
        ...
        if "trajectory_scores" in pred:
            # Multi-positive InfoNCE over active UIDM states.  The target is
            # the expression's object set, while other active states are
            # wrong trajectories.  This is an auxiliary signal; UIDM still
            # owns the actual assignment and memory transition.
            traj_scores = pred["trajectory_scores"]
            traj_losses = []
            for b, c in enumerate(clips):
                if c.get("domain") != "rmot":
                    continue
                fr_target = {str(x) for x in c["frames"][f].get(
                    "target_gids", [])}
                if not fr_target:
                    continue
                target_slots = set()
                for gid in fr_target:
                    si = self.gid_maps[b].get(gid)
                    if si is not None and bool(self.active[b, si]):
                        target_slots.add(int(si))
                active_slots = self.active[b].nonzero().flatten().tolist()
                negative_slots = [si for si in active_slots
                                  if si not in target_slots]
                if not target_slots or not negative_slots:
                    continue
                valid_slots = torch.as_tensor(active_slots, dtype=torch.long,
                                              device=dev)
                pos_slots = torch.as_tensor(sorted(target_slots),
                                            dtype=torch.long, device=dev)
                score = traj_scores[b, valid_slots]
                pos_score = traj_scores[b, pos_slots]
                traj_losses.append(-torch.logsumexp(pos_score, dim=0)
                                   + torch.logsumexp(score, dim=0))
            if traj_losses:
                losses["loss_traj"] = torch.stack(traj_losses).mean()
            else:
                losses["loss_traj"] = traj_scores.sum() * 0.0
