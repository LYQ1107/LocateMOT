#!/usr/bin/env python3
"""L87-B internal full-video inference under corrected candidate-vs-NULL rule."""
...
def main() -> int:
    ...
    if out.exists() and any(out.iterdir()):
        raise FileExistsError(f"refusing nonempty L87-B full-video output: {out}")
    ...
    try:
        ...
        try:
            for dataset in datasets:
                ...
                for video in INTERNAL[dataset]:
                    ...
                    paths["tracker_data"] = out / dataset / "trackers" / "l87b" / "data"
                    ...
                    for frame_index, group in enumerate(groups):
                        ...
                        for record in records:
                            ...
                            if not np.isfinite(scores).all(): raise FloatingPointError("nonfinite L87-B score")
                            ...
                            audit = {"format": "locatemot-l87b-fullvideo-prediction-audit-v1",
                    ...
                    print(f"[l87b-infer] {dataset} video={video} frames={len(groups)} queries={len(queries)} elapsed={time.perf_counter()-started:.1f}s", flush=True)
        finally:
        ...
        summary = {"format": "locatemot-l87b-fullvideo-inference-v1", "status": "complete",
                   ...
                   "next_action": "run independent corrected L87-B TrackEval wrapper"}
        write_json(out / "summary.json", summary); write_json(out / "provenance.json", summary); write_json(out / "status.json", summary)
        return 0
    except Exception:
        trace = traceback.format_exc(); (out / "INCOMPLETE.md").write_text("# L87-B full-video inference — INCOMPLETE\n\n" + trace)
        write_json(out / "status.json", {"format": "locatemot-l87b-fullvideo-inference-v1", "status": "incomplete",
