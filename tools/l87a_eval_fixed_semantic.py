#!/usr/bin/env python3
"""Fixed 16-calibration/24-validation evaluation for corrected L87-A policy."""
...
def main() -> int:
    ...
    if out.exists() and any(out.iterdir()):
        raise FileExistsError(f"refusing nonempty L87-A semantic output: {out}")
    ...
    try:
        ...
        if len(preselection) != 40 or [int(row["fixed_eval_order"]) for row in preselection] != list(range(40)):
            raise AssertionError("L87-A fixed preselection order drift")
        ...
        preselection_audit = {
            "format": "locatemot-l87a-preselection-label-isolation-v1", "status": "complete",
        ...
        semantic = {
            "format": "locatemot-l87a-fixed-semantic-v1", "status": "complete", "decision": decision,
        ...
        gate = {"format": "locatemot-l87a-fixed-gate-v1", "status": decision, "decision": decision,
        ...
        provenance = {"format": "locatemot-l87a-fixed-semantic-provenance-v1", "status": "complete",
                      "command": command, "work_root": str(WORK_ROOT), "asset_root": str(ASSET_ROOT),
        ...
        write_json(out / "semantic.json", semantic); write_json(out / "gate_decision.json", gate)
        write_json(out / "provenance.json", provenance); write_json(out / "status.json", {"format": "locatemot-l87a-fixed-semantic-status-v1",
        ...
        trace = traceback.format_exc(); (out / "INCOMPLETE.md").write_text("# L87-A fixed semantic — INCOMPLETE\n\n" + trace)
        write_json(out / "status.json", {"format": "locatemot-l87a-fixed-semantic-status-v1", "status": "incomplete",
