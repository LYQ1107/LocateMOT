...
def main():
    ...
    sr1 = subset_rows.get("dla|T1", {})
    sr2 = subset_rows.get("dla|T2", {})
    ...
    status = {
        "stage": "L1-A",
        "decision": decision,
        "assa_delta_pp": round(assa_delta * 100, 2),
        "idsw_delta_pct": round(idsw_delta_pct, 1),
        "hota_delta_pp": round(hota_delta * 100, 2),
        "idf1_delta_pp": round(idf1_delta * 100, 2),
    ...
    lines.append(f"在固定 detections 下比较 T0 IoU → T6 trajectory-aware association。"
                f"T6 vs T0：AssA {fmt(assa_delta * 100, 2)} pp，IDSW {fmt(idsw_delta_pct, 1)}%，"
                f"HOTA {fmt(hota_delta * 100, 2)} pp。LocateAnything Recall@0.5 = {recall.get('recall_0.5', 'n/a')}。\n")
    ...
    lines.append(f"T6 vs T0: AssA {fmt((num(g(t6,'HOTA_AssA(0)'))-num(g(t0,'HOTA_AssA(0)')))*100,2)}pp, "
                 f"IDSW {fmt(idsw_delta_pct,1)}%, HOTA {fmt(hota_delta*100,2)}pp；"
                 f"T6 vs T1: AssA {fmt((num(g(t6,'HOTA_AssA(0)'))-num(g(t1,'HOTA_AssA(0)')))*100,2)}pp, "
    ...
    h.append(f"DanceTrack val 连续帧低 IoU 事件极少：T0 的 <0.1 仅 {sr0.get('iou_<0.1_n','n/a')} 个、"
             f"0.1-0.3 仅 {sr0.get('iou_0.1-0.3_n','n/a')} 个、0.3-0.5 仅 {sr0.get('iou_0.3-0.5_n','n/a')} 个"
             f"（占总连续匹配 86,906 的 0.12%）。T0 vs T6：0.3-0.5 acc "
             f"{sr0.get('iou_0.3-0.5_acc','n/a')} vs {sr.get('iou_0.3-0.5_acc','n/a')}；≥0.5 acc "
             f"{sr0.get('iou_>=0.5_acc','n/a')} vs {sr.get('iou_>=0.5_acc','n/a')}。"
             f"说明本 split+detector 组合下 IoU 失效场景很少，无法用 DanceTrack val 证明 trajectory 价值。\n")
    h.append("## 9. High-density 结果\n")
    h.append(f"T0 low/med/high acc：{sr0.get('density_low_acc','n/a')}/"
             f"{sr0.get('density_medium_acc','n/a')}/{sr0.get('density_high_acc','n/a')}；"
             f"T6：{sr.get('density_low_acc','n/a')}/{sr.get('density_medium_acc','n/a')}/"
             f"{sr.get('density_high_acc','n/a')}。T6 在高中低密度都略低于 T0；ambiguous acc "
             f"T0 {sr0.get('ambiguous_acc','n/a')}（n={sr0.get('ambiguous_n','n/a')}）vs "
             f"T6 {sr.get('ambiguous_acc','n/a')}（n={sr.get('ambiguous_n','n/a')}）。\n")
    h.append("## 10. Reactivation\n")
    h.append(f"T0 events={sr0.get('reactivation_events','n/a')}，id kept={sr0.get('reactivation_accuracy','n/a')}，"
             f"mean gap={sr0.get('reactivation_mean_gap','n/a')}；T1 events={sr1.get('reactivation_events','n/a')}，"
             f"id kept={sr1.get('reactivation_accuracy','n/a')}；T2 events={sr2.get('reactivation_events','n/a')}，"
             f"id kept={sr2.get('reactivation_accuracy','n/a')}；T6 events={sr.get('reactivation_events','n/a')}，"
             f"id kept={sr.get('reactivation_accuracy','n/a')}，mean gap={sr.get('reactivation_mean_gap','n/a')}。"
             f"T6 以 4.3 帧的平均 gap 高频 reactivate，但只有 6.3% 保持了原 ID，说明 reactivation 在抢夺身份而不是恢复身份。\n")
    h.append("## 11. T6 vs IoU\n")
    h.append(f"AssA {fmt(assa_delta*100,2)}pp，IDF1 {fmt(idf1_delta*100,2)}pp，IDSW {fmt(idsw_delta_pct,1)}%，HOTA {fmt(hota_delta*100,2)}pp。\n")
    h.append("## 12. T6 vs Kalman/OC-SORT\n")
    h.append(f"T6 vs T1：AssA {fmt((num(g(t6,'HOTA_AssA(0)'))-num(g(t1,'HOTA_AssA(0)')))*100,2)}pp，"
             f"IDSW {fmt(num(g(t6,'CLEAR_IDSW'))-num(g(t1,'CLEAR_IDSW')),0)}。\n")
    h.append("## 13. T6 vs B6-local\n")
    h.append(f"T6 vs T2：AssA {fmt((num(g(t6,'HOTA_AssA(0)'))-num(g(t2,'HOTA_AssA(0)')))*100,2)}pp，"
             f"IDF1 {fmt((num(g(t6,'Identity_IDF1'))-num(g(t2,'Identity_IDF1')))*100,2)}pp，"
             f"IDSW {fmt(num(g(t6,'CLEAR_IDSW'))-num(g(t2,'CLEAR_IDSW')),0)}；"
             f"至少两项更优：{beats_t2}。\n")
    h.append(f"## 14. 是否真正证明“为什么不用 IoU”\n{decision}\n")
    h.append("## 15. 剩余最大瓶颈\n")
    h.append("1) Identity 一致性：T4-T6 的 IDSW 3,329→5,962 远高于 T0 879；AssA 0.163→0.227 低于 T0 0.422。"
             "2) Reactivation 无有效身份门控：T6 reactivation id-kept 仅 6.3%。"
             "3) DanceTrack val 低 IoU 场景极少（<0.5 仅 0.12%），trajectory 的价值场景没有被该协议覆盖。"
             "4) D-LA 检测碎片化（DetA 0.381 且 FN 140k）被 T6 大幅修复，但代价是 ID 混乱；"
             "D-CTRL YOLOX 下 T0/T1 的 IDSW 也高达 1,393-1,569，碎片化是比低 IoU 更主要的关联压力。\n")
    h.append("## 16. 下一阶段唯一建议\n")
    h.append("不要继续堆 memory/reactivation 复杂度。先做 identity-preserving association 修复："
             "（a）reactivation 增加严格相似度/运动门控并限制每次复活消耗；"
             "（b）把 ID-consistency loss（跨帧同 ID 判别）纳入训练；"
             "（c）在 D-CTRL 强检测下重跑 T2-T6 以分离检测碎片与关联能力。"
             "在 AssA/IDF1/IDSW 三项至少两项明确超过 T0 之前，不进入 Visual Prompt LoRA。\n")
    h.append("## 17. Detection 质量（D-LA）\n")
    h.append(f"LocateAnything-3B person query 在 pilot 90 帧上 Recall@0.3={recall.get('recall_0.3','n/a')}，"
             f"Recall@0.5={recall.get('recall_0.5','n/a')}，Recall@0.7={recall.get('recall_0.7','n/a')}，"
             f"candidates/frame={recall.get('candidates_per_frame','n/a')}，FPS={recall.get('fps','n/a')}，"
             f"peak VRAM={recall.get('peak_gpu_gb','n/a')}GB。d1（'person.'）与 d3（'people.'）接近且远好于 d2（'a person.'）。\n")
    h.append("## 18. D-CTRL 结果（YOLOX-X DanceTrack 官方权重）\n")
    h.append("D-CTRL 只运行 T0/T1（T2-T6 需要 ObjectToken 特征，未在 YOLOX 框上做特征迁移）："
             f"T0 HOTA={fmt(g(ctrl_rows.get('T0',{}),'HOTA_HOTA(0)'))}，DetA={fmt(g(ctrl_rows.get('T0',{}),'HOTA_DetA(0)'))}，"
             f"AssA={fmt(g(ctrl_rows.get('T0',{}),'HOTA_AssA(0)'))}，IDF1={fmt(g(ctrl_rows.get('T0',{}),'Identity_IDF1'))}，"
             f"IDSW={fmt(g(ctrl_rows.get('T0',{}),'CLEAR_IDSW'),0)}；"
             f"T1 HOTA={fmt(g(ctrl_rows.get('T1',{}),'HOTA_HOTA(0)'))}，AssA={fmt(g(ctrl_rows.get('T1',{}),'HOTA_AssA(0)'))}，"
             f"IDF1={fmt(g(ctrl_rows.get('T1',{}),'Identity_IDF1'))}，IDSW={fmt(g(ctrl_rows.get('T1',{}),'CLEAR_IDSW'),0)}。"
             "固定强检测下 IDSW 依然上千，说明检测碎片本身是关联压力的主要来源之一。\n")
    h.append("## 19. 训练设置与稳定性\n")
    h.append("TemporalBundle（TrajectoryEncoder K=8 两层因果 transformer + MotionPredictor MLP + MemoryFusion + "
             "Motion/Reactivation residual heads + nm_bias）在 32 个 train 视频、每视频 250 个 hard 样本上训练 "
             "2 epochs × 8,000 steps = 16,000 steps；AdamW lr=2e-4、weight_decay=1e-4、grad clip 1.0、cosine schedule；"
             "B6 全程冻结。loss 主体从 ~6 降到 ~1.2-2.5；部分困难样本出现有限大 spike（最高 ~1,500，BCE 对极端 logit），"
             "未产生 NaN。训练中修复了两个数值 bug：degenerate box 的 geometry inf（normalize_geom clamp）"
             "与轨迹注意力 padding 位置 NaN（对角线自注意力）。\n")
    h.append("## 20. 推理速度与资源\n")
    h.append("D-LA val 25,508 帧：T0 405-672 FPS、T1 233-615 FPS、T2 9-26 FPS、T3 5-16 FPS、T4 4-12 FPS、"
             "T5 5-10 FPS、T6 3-9 FPS（不同轮次波动）。缓存阶段 LocateAnything 3B 约 0.3 FPS/GPU、峰值显存 ~10.7GB；"
             "D-LA 全量缓存 67,304 帧约 14-20GB；系统内存全程控制在 65-117GB 可用，无新的 OOM。\n")
    h.append("## 21. 科学解释与边界\n")
    h.append("T4-T6 通过 motion/reactivation 显著降低了 FN（T6 FN=26,686 vs T0 140,477）并提高 MOTA/DetA，"
             "但代价是 AssA/IDF1/IDSW 全面劣化：learned 关联把‘多检测到’与‘保持同一身份’变成了竞争关系。"
             "这不能证明 trajectory modeling 无用，只能证明当前实现（尤其是 reactivation 的 ID 复用策略）"
             "没有做好 identity 一致性。结论边界：仅适用于 DanceTrack val + D-LA person 检测协议；"
             "不能外推到其他数据集或 open-vocabulary 场景；没有改动 LocateAnything/B6 权重。\n")
    h.append("## 22. 遗留问题与后续验证\n")
    h.append("（1）D-CTRL 下 T2-T6 未跑，无法完全分离 detection 与 association 的贡献；"
             "（2）低 IoU 事件不足，需要 MOT17/MOT20 或故意稀疏检测（drop 帧）协议来制造 trajectory 必须的场景；"
             "（3）reactivation 的相似度门控与 ID 一致性损失是第一优先修复项；"
             "（4）训练 loss 的大 spike 值得做 loss 截断/标签清洗后再训练。\n")
    h.append("## 23. D-LA 完整指标表（官方 TrackEval，val 25 视频）\n")
    h.append("| variant | HOTA | DetA | AssA | LocA | MOTA | MOTP | IDF1 | IDP | IDR | IDSW | FP | FN | Frag | MT | PT | ML |\n")
    h.append("|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|\n")
    for v in ["T0", "T1", "T2", "T3", "T4", "T5", "T6"]:
        r = main_rows.get(v, {})
        h.append(f"| {v} | {fmt(g(r,'HOTA_HOTA(0)'))} | {fmt(g(r,'HOTA_DetA(0)'))} | {fmt(g(r,'HOTA_AssA(0)'))} "
                 ...
                 f"| {fmt(g(r,'CLEAR_Frag'),1)} | {fmt(g(r,'CLEAR_MT'),0)} | {fmt(g(r,'CLEAR_PT'),0)} "
                 f"| {fmt(g(r,'CLEAR_ML'),0)} |\n")
    h.append("逐级解读：T2 在两帧 B6 迁移到全视频后 AssA 0.163/IDSW 3,329 明显差于 T0（0.422/879），"
             "说明 B6 的 no-match/ID 行为在连续视频中碎片化；T3 加入 trajectory 后仅把 DetA 从 0.372 提到 0.386，"
             "AssA 基本不变；T4 的 motion 把 MOTA 从 0.361 拉到 0.559（FN 从 139k 降到 93k），但 IDSW 升到 4,107；"
             "T5 memory 与 T4 持平；T6 reactivation 把 MOTA 推到 0.840、DetA 0.888、FN 26,686，"
             "但 AssA 0.227、IDF1 0.349、IDSW 5,962 全面劣于 T0。\n")
