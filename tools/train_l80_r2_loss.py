#!/usr/bin/env python3
"""Bounded L80-R2 probe: train-only hard-negative/multi-positive loss change."""
...
def strict_reload(path: Path, device: torch.device) -> dict[str, Any]:
    ...
    if result.missing_keys or result.unexpected_keys:
        raise AssertionError(f"R2 strict reload mismatch: {result}")
    model.eval()
...
def save_checkpoint(out: Path, model: L80RawRegionCorrespondence,
                    optimizer: torch.optim.Optimizer, step: int, args: argparse.Namespace) -> Path:
    path = out / f"checkpoint_l80_r2_step{int(step)}.pt"
    torch.save({
        "format": "locatemot-l80-r2-loss-checkpoint-v1", "stage": str(args.stage),
        "step": int(step), "seed": int(args.seed), "model_config": model.config.__dict__,
        "model_state_dict": model.state_dict(), "optimizer_state_dict": optimizer.state_dict(),
        "model_parameter_count": int(sum(p.numel() for p in model.parameters())),
        "region_interface": "unchanged L80-R0 three-tap 4x4/2x2/scene interface",
        "loss_variant": "detached current-logit all-negative hard mining plus explicit positive floor",
    }, path)
    return path
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if out.exists() and any(out.iterdir()):
        raise FileExistsError(f"refusing nonempty R2 training output: {out}")
    ...
    if args.steps <= 0:
        raise ValueError("R2 requires positive --steps")
    ...
    domain_counts: Counter[str] = Counter(); category_counts: Counter[str] = Counter()
    ...
    try:
        ...
        for item in schedule:
            ...
            if FORBIDDEN_LABEL_FIELDS.intersection(metadata):
                raise AssertionError(f"R2 label leaked before feature construction: {item['unit_key']}")
            batch = store.build_unit(metadata)
            digest = row_digest(batch.row_keys)
            if key_digests.setdefault(batch.unit_key, digest) != digest:
                raise AssertionError(f"R2 candidate key drift: {batch.unit_key}")
            raw = raw_inputs_for_unit(clip_model, batch, device, cache)
            if tuple(raw["visual_tokens"].shape) != (batch.candidate_count, 63, 768):
                raise AssertionError(f"R2 visual shape drift: {batch.unit_key}")
            # No membership label is opened until the complete native feature
            # record has been constructed and row order has been verified.
            full = label_cache.get(batch.unit_key)
            if full is None:
                full = load_full_unit_for_labels(batch.unit_key); label_cache[batch.unit_key] = full
            ...
            loss, parts = l80_r2_loss(output, labels["labels"], labels["coverage_mask"], observations,
                                      history_mask, labels["category"])
            if not bool(torch.isfinite(loss)):
                raise FloatingPointError(f"R2 nonfinite loss at {batch.unit_key}")
            optimizer.zero_grad(set_to_none=True); loss.backward()
            finite_grad = True; any_nonzero = False
            ...
            if not finite_grad or not any_nonzero:
                raise FloatingPointError(f"R2 invalid trainable gradient at {batch.unit_key}")
            grad_norm = float(torch.nn.utils.clip_grad_norm_(model.parameters(), float(args.grad_clip)))
            optimizer.step(); step += 1; finite_steps += 1; nonzero_steps += int(any_nonzero)
            ...
            loss_trace.append({"step": step, "loss_finite": True, "gradient_finite": True,
                               ...
                               "candidate_count": int(batch.candidate_count), "candidate_key_digest": digest,
                               "visual_forward_count": int(cache.visual_forward_count), **parts})
            sampling_trace.append({"step": step, "unit_key": batch.unit_key, "dataset": batch.dataset,
                                   ...
                                   "candidate_count": int(batch.candidate_count), "positive_count": int(labels["positive_count"]),
                                   "schedule_position": int(item["schedule_position"]), "schedule_kind": item["schedule_kind"],
        ...
        if step != args.steps:
            raise AssertionError(f"R2 step count mismatch {step} != {args.steps}")
    except Exception:
        (out / "INCOMPLETE.md").write_text("# L80-R2 training — INCOMPLETE\n\n" + traceback.format_exc() +
        ...
        if clip_model is not None: del clip_model
        gc.collect()
        if device.type == "cuda": torch.cuda.empty_cache()
    ...
    metrics = {
        "format": "locatemot-l80-r2-fit-metrics-v1", "status": "complete", "stage": str(args.stage),
        ...
        "fit_units_available": len(rows), "domains_seen": dict(domain_counts), "categories_seen": dict(category_counts),
        "model": model.parameter_report(), "model_parameter_count": int(sum(p.numel() for p in model.parameters())),
        "region_interface": "unchanged R0 3x(4x4 ROI+2x2 context+scene)",
        "loss_variant": {"hard_negative": "detached current membership logits, all-negative fallback, max 16",
                         "pair_margin": 0.75, "positive_floor": 0.60, "minimum_positive_margin": 0.75},
        "learning_rate": float(args.lr), "weight_decay": float(args.weight_decay),
        "candidate_rows_complete": True, "candidate_key_drift": 0, "candidate_deletion": False,
        "candidate_truncation": False, "raw_dense_cache_written": False,
        "persistent_frame_feature_cache": False, "same_class_hard_negative_metadata": "unavailable; all-negative fallback",
    ...
    write_json(out / "metrics_l80_r2_fit.json", metrics)
    write_json(out / "config.json", {"format": "locatemot-l80-r2-fit-config-v1", "stage": args.stage,
        ...
        "loss_variant": "R2 detached current-logit all-negative hard mining plus all-positive floor/listwise",
        "same_class_hard_negative_metadata": "unavailable; all-negative fallback", "screening_gt_used": False,
        "official_test_labels_read": False, "ordinary_mot_ovmot_touched": False, "hota_trackeval_run": False})
    write_json(out / "reload_audit.json", {"format": "locatemot-l80-r2-reload-audit-v1", "status": "complete",
    ...
    write_json(out / "provenance.json", {"format": "locatemot-l80-r2-fit-provenance-v1", "status": "complete",
        ...
        "inputs": {"manifest": str(MANIFEST), "manifest_sha256": sha256_file(MANIFEST),
                   ...
                   "clip_weight": str(CLIP_WEIGHT), "clip_sha256": sha256_file(CLIP_WEIGHT)},
        "outputs": [str(out / "metrics_l80_r2_fit.json"), str(out / "loss_trace.json"), str(out / "sampling_trace.json")],
        "fit_only": True, "only_changed_factor": "train-only hard-negative/multi-positive loss",
        "labels_attached_after_raw_feature_construction": True, "token_span_region_alignment": "UNALIGNED",
        "static_motion_alignment": "UNALIGNED", "screening_gt_used": False, "official_test_labels_read": False,
        "ordinary_mot_ovmot_touched": False, "hota_trackeval_run": False, "old_assets_modified": False})
    write_json(out / "status.json", {"format": "locatemot-l80-r2-status-v1", "status": "complete",
        ...
        "failure_root_cause": None, "next_action": "run fixed R2 semantic evaluation",
        "screening_gt_used": False, "official_test_labels_read": False,
        "ordinary_mot_ovmot_touched": False, "hota_trackeval_run": False})
    return metrics
...
def main() -> int:
    ...
    parser.add_argument("--out", type=Path, required=True); parser.add_argument("--steps", type=int, required=True)
    parser.add_argument("--save-steps", default="100,500"); parser.add_argument("--seed", type=int, default=SEED)
    parser.add_argument("--device", default="cuda:0"); parser.add_argument("--stage", default="r2-loss-probe")
    parser.add_argument("--lr", type=float, default=2e-4); parser.add_argument("--weight-decay", type=float, default=1e-4)
    parser.add_argument("--grad-clip", type=float, default=5.0); parser.add_argument("--cache-items", type=int, default=16)
    args = parser.parse_args(); print(json.dumps(run(args), indent=2), flush=True); return 0
