...
def run_tracker(model, by_video, clip_dir, out_dir, gpu, spec_emb,
    ...
    for vi, (vid, entries) in enumerate(by_video.items()):
        ...
        tracker.uidm_spec_valid = float(spec_valid)
...
def main():
    ...
    ap.add_argument(
        "--spec-null", action="store_true",
        help="run the closed-set MOT path with an explicit null specification")
    args = ap.parse_args()
    ...
    model = L8UnifiedUIDM(**SIZES[cfg.get("model", "base")],
                          ...
                          trajectory_memory=cfg.get("trajectory_memory", True))
    ...
    for key in args.domains.split(","):
        ...
        spec_emb = (np.zeros(512, np.float32) if args.spec_null else
                    _specs([spec_text], device="cpu")[0])
        spec_valid = 0.0 if args.spec_null else 1.0
        ...
        run_tracker(model, by_video, CLIP_EVAL / clip_dom, src_dir,
                    args.gpu, spec_emb, args.ablation, spec_valid)
