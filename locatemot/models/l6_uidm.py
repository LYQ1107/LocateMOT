...
class UIDM(nn.Module):
    ...
    def forward_frame(self, frame):
        ...
        if self.use_cue_rel and getattr(self, "use_cue_mix", True):
            ...
            mask_value = torch.finfo(cue_scores.dtype).min
            cue_scores = cue_scores.masked_fill(
                ~cand_mask.unsqueeze(1).unsqueeze(-1), mask_value)
            cue_scores = cue_scores.masked_fill(
                ~trk_mask.unsqueeze(2).unsqueeze(-1), mask_value)
            rel_ctx = torch.stack([
        ...
        pair_logits = pair_logits.masked_fill(~cand_mask.unsqueeze(1),
                                              mask_value)
        pair_logits = pair_logits.masked_fill(~trk_mask.unsqueeze(2),
                                              mask_value)

        no_match = self.no_match_head(
            torch.cat([trk_out, track_feat], dim=-1)).squeeze(-1)
        no_match = no_match.masked_fill(~trk_mask, mask_value)
        new = self.new_head(
            torch.cat([cand_out, cand_feat], dim=-1)).squeeze(-1)
        new = new.masked_fill(~cand_mask, mask_value)
...
def uidm_frame_loss(frame, pred, tgt):
    ...
    if cv.any():
        if new_w is not None:
            loss_new = wbce(new[cv], new_t[cv].float(), new_w[cv])
        elif col_w is not None:
