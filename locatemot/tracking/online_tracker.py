    ...
    def _associate_uidm(self, tracks, cur_feats, cur_boxes, frame_id):
            ...
            with torch.no_grad():
                try:
                    sem, _ = self.uidm_adapter(
                        ...
                        torch.as_tensor(clip_arr, device=self.device), spec,
                        spec_valid=getattr(self, "uidm_spec_valid", None))
                except TypeError:
                    # Compatibility with pre-L14 adapters loaded by the
                    # historical L13 evaluator.
                    sem, _ = self.uidm_adapter(
                        torch.as_tensor(pbd_arr, device=self.device),
                        torch.as_tensor(clip_arr, device=self.device), spec)
        ...
        for i, trk in enumerate(tracks):
            ...
            if trk.anchor_features is not None and \
                ...
                anchor[i] = ref[i]
        if self.uidm_adapter is not None and hasattr(
                self.uidm_adapter, "track_condition"):
            with torch.no_grad():
                h_t = torch.as_tensor(h[None], device=self.device)
                cond = self.uidm_adapter.track_condition(
                    h_t, spec,
                    spec_valid=getattr(self, "uidm_spec_valid", None))
                track_delta = cond[0] if isinstance(cond, tuple) else cond
                track_delta = track_delta[0].cpu().numpy().astype(np.float32)
        cb = np.asarray(cur_boxes, dtype=np.float64).reshape(N, 4)
        feats = compute_affinity_features(
            tb, cb, ref, anchor, cp, cg, gaps, ages, hits, pb,
            self.l1d_weights, image_size,
        ...
        batch = {
            "trk_tok": torch.as_tensor(
                (h + (track_delta if track_delta is not None else 0.0))[None],
                device=self.device),
            "cand_pbd": torch.as_tensor(cp[None], device=self.device),
